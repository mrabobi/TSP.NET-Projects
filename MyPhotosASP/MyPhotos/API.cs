...
namespace MyPhotos
{
    public class API
    {
        ...
        {
            //return (from pic in dbcontext.PicTableSet.Include(p => p.Categories)
            //       select pic);
             return dbcontext.PicTableSet.Include(p=>p.Person).Include(c=>c.Categories).ToList();
        }
        
        public List<Categories> GetCategoriesById(int id)
        {
            return dbcontext.CategoriesSet.Where(c => c.ID_Category == id).ToList();
        }
